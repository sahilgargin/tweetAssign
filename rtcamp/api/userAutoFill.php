...
if (!$link) {
    ...
    if ($_GET['kw']) {
        $result = mysqli_query($link, "SELECT * from newDd.followers where oauth_uid = '" . $_GET['tw'] . "' and name_followers like '%" . $kw . "%' ");
        ...
        while ($row = mysqli_fetch_assoc($result)) {
            ...
            $res['row'][] = array(
                'id' => $row['id_followers'],
                'name' => $row['name_followers'],
                'tweets' => json_decode($row['tweet_followers'])
            );
        }
        header('Content-Type: application/json');
        $count              = mysqli_num_rows($result);
